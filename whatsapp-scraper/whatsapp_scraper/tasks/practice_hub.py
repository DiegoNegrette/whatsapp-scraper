...
def sync_missing_phone_numbers():
    ...
    while current_page <= total_pages:
        ...
        if not r.status_code == 200:
            ...
            return
        response = r.json()
        total_pages = math.ceil(response["total_entries"] / 100)
        current_page += 1
    ...
    for appointment in target_appointments:
        patient_info = patient_id_to_patient_info_dict.get(appointment.patient_id)
        selected_phone_number = None
        if patient_info and len(patient_info["numbers"]) >= 1:
            for phone_info in patient_info["numbers"]:
        ...
        if selected_phone_number:
            ...
            appointment.language = patient_info["locale"]
